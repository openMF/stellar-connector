...
@Component
public class MifosBridgeAccountAdjuster {
  public static final int STELLAR_TRANSFER_ACCOUNT = 1; //TODO: ???
  private final RestAdapterProvider restAdapterProvider;

  @Autowired
  MifosBridgeAccountAdjuster(final RestAdapterProvider restAdapterProvider)
  {
  ...
  public void tellMifosPaymentSucceeded(
      ...
      final BigDecimal amount) throws MifosBridgeAccountAdjustmentFailedException
  {
    try {
    ...
    {
      throw new MifosBridgeAccountAdjustmentFailedException(ex.getResponse().getReason());
    }
  }
  ...
  public void informMifosOfIncomingStellarPayment(
      ...
      final String assetCode,
      final Long eventId) throws MifosBridgeAccountAdjustmentFailedException
  {
    try {
      final RestAdapter restAdapter = this.restAdapterProvider.get(endpoint);
      final MifosClientService clientService = restAdapter.create(MifosClientService.class);
